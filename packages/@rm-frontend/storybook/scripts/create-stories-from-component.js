...
const templateStory = async (component, componentDir, mod) => {
  ...
  return render('Component.stories.tsx.njk', options)
    .then((content) =>
      fs.outputFile(
        `packages/@rm-frontend/storybook/tmp/${mod}/${componentPascalcased}.stories.tsx`,
        content
      )
    )
    .catch((err) => {
...
module.exports = async () => {
  const exportsAbsolute = await glob(
    'packages/@rm-frontend/*/lib/exports.json'
  );
  ...
  exportsRelative.forEach(async (e) => {
    ...
    const schemaPaths = await glob(
      `packages/@rm-frontend/${mod}/lib/{${exportsKeys.join(
        ','
