...
As with the adaptation process, the big assumption here is that you already have a **kickstartDS** base component in mind. But unlike in that one, we have some requirement or feature in mind that is not covered by it. We _almost_ have a match.
...
If you don't have **kickstartDS** base component in mind yet, you're probably better served by our guide ["Create a component"](./create.mdx). In that guide, you'll start off without a specific component in mind. This means defining a structure (someting you probably have an idea about already) first, and then mapping that structure to a fitting component second.
...
There should be an exception though, as we have a requirement on mind that is not part of the component yet. This will probably be part of the purpose of our component.
...
https://www.kickstartds.com/storybook/?path=/story/base-headline--h-1
...
For an abridged version of that process, have a look at the `Headline` again (adding our own `switchOrder`):
...
In genera: these guides are mainly meant as basic building blocks, that can be combined and remixed by you to create exactly what you need... even if that's something more complex as described in these basic guides here.

</Admonition>

For the full version of customizing a `Headline` have a look at our guide ["Customize `Headline` component"](../examples/components/headline.mdx).
