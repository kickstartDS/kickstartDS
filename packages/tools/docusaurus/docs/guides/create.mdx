...
```bash
$ yarn add @kickstartds/base@2.0.0-canary.922.4908.0
```
...
Finally, to test the integration works as expected, we'll want to create one additional file: `sd.config.js`. As the name may imply, this is further configuration for our Design Tokens (more specifically the Style Dictionary that gets used under the hood) to convert them into different outputs:
...
We basically use the configuration provided by... TODO how does correct, current `sd.config.cjs` look like? Shouldn't it be `@kickstartds/style-dictionary`?
TODO add description which paths are configured here
TODO can Storybook / any task be actively stopped from running by extension? return `false`?

<br />
Read more about [our usage of it](#) TODO add link or [how you can benefit](#) TODO
add link.
