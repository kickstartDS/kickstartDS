...
Enter the prompted information, and you'll end up with `package.json` describing your freshly made `npm` package. Use `dist/index.js` for the [entry point](focus://5), this is where our bundled code will live. The rest is pretty much up to your liking! It should look roughly like this:
...
This will leave us with the following files for now:
...
We'll continue by applying our branding to **kickstartDS**, which result in a bunch of token JSON files being added. This will also give us the opportunity to set up `Storybook`, and get a first visual representation of our Design System in the browser!
