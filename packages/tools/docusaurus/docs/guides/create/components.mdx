...
- **`Headline`** based on `@kickstartds/base/lib/headline`:<br/>Adopt existing component (with a reduced set of props), add custom field `switchOrder`
- **`Teaser Card`** based on `@kickstartds/base/lib/teaser-box`:<br/>Adopt existing component (with a reduced set of props), reduce complexity by making `link` a simple string... hard-code rest of `Button` props
- **`Section`** based on `@kickstartds/base/lib/section`:<br/>Adopt existing component (with a reduced set of props), add custom `background` to showcase extending existing property / `enum`. Also add `link` (display at the end of the `Section`), and let it take multiple entries (1. primary / 2. secondary / 3.+ outline / ghost)
