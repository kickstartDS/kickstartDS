...
```json package.json focus=7
{
  ...
  "main": "dist/index.js",
  "scripts": {
    "build-storybook": "yarn build-tokens && run-p sass-manager schema && storybook build",
    "build-tokens": "kickstartDS tokens compile",
    "init-tokens": "kickstartDS tokens init",
    "sass-manager": "sass --load-path=node_modules .storybook/manager.scss static/manager.css",
    "schema": "run-p schema:*",
...
We have a [dedicated guide for customization](../components/customize.mdx), too, explaining the concepts a bit more in-depth.
