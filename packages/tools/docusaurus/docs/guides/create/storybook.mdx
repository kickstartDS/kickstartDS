...
To see all options available on a `Button`, have a look at its documentation in `Storybook` (either [in our docs](https://www.kickstartds.com/storybook/?path=/docs/base-button--solid), or locally through the composed docs in your sidebar TODO add link to composition above).

If you re-check your *Welcome* page in the browser again now, you'll see the button having space reserved for an `Icon`. But the `Icon` itself is still missing. This is because we haven't included our **Icon Sprite** for component previews in **Storybook**, yet. The only reason they're successfully displayed on your **Icons** page currently, is that the `storybook-design-token` addon is picking them up using its own loader (TODO add link to `storybook-design-token` addon configuration section).

TODO add picture of `Button` with reserved spacing

Let's add our **Icon Sprite** to **Storybook** for real, now!

#### Integrate `Icons` with **Storybook** previews

TODO add short motivational description of why we use `IconSprites`, and link to IconSprite / Icons-docs
...
TODO add screenshot of `Button` **with** `Icon`
...
We use a small, light-weight communication component called `radio` (basically a mini [PubSub](https://en.wikipedia.org/wiki/Publish%E2%80%93subscribe_pattern) TODO check that this is actually true) in all existing components for communication and event handling. It's part of our **Core** (`@kickstartDS/core`) architecture (TODO link article about `radio`). To make debugging easier and more transparent, we connect our `radio` with **Storybook**s actions, and their actions addon.
...
With that, we've successfully added **Storybook** to our Design System, integrated it with our **Design Token** / **Style Dictionary**, adopted some concepts of **kickstartDS**, and even added a bit of theming!

## Result

TODO describe result, maybe cut this from all sub pages... tbd

## Next Step

Now that all the general setup is out of our way, we'll get to the fun part. Filling our Design System with life, adding our own components!
