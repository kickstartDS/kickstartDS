...
We start by creating a new file: `branding-token.json` at our project root:

```bash
touch branding-token.json
```
...
TODO add presenters for token values to make this nicer
...
```json branding-token.json focus=2:4
{
  "color": {
    "primary": "#05566a",
...
```json branding-token.json focus=2:4,6:8
{
  "color": {
    "primary": "#05566a",
    "primary-inverted": "#ecff00",
...
```json branding-token.json focus=2:4,6:8,11:12
{
  "color": {
    "primary": "#05566a",
    "primary-inverted": "#ecff00",
...
```json branding-token.json focus=18:20,29:30
{
  "color": {
    "primary": "#05566a",
    "primary-inverted": "#ecff00",
...
```json branding-token.json focus=63,65:66
{
  "color": {
    "primary": "#05566a",
    "primary-inverted": "#ecff00",
...
```
yarn run v1.22.18
$ kickstartDS tokens init --cleanup --rc-only .txt
  _    _      _        _             _   ____  ____
...
[info] successfully generated primitives tokens and wrote them to folder /tmp/tokens-init-init-task/tokens
...
It uses your `branding-token.json` as an input, and generates a folder `tokens` for you. That folder holds `JSON` files defining your Design Token by category:
...
This is based on `Style Dictionary`, so generally follows [their configuration scheme](https://amzn.github.io/style-dictionary/#/config?id=using-configuration-files).

Let's details some lines:
...
We configure the default Design Token location, which is `tokens` as a glob `tokens/**/*.json`...
...
<Admonition type="tip" title="To commit, or not to commit...">
  It's up to personal / team preference if the generated `token.css` file should be added to your repository... or your `.gitignore`.
...
This can be done by configuring the various `.json` files found within the `tokens` folder.
