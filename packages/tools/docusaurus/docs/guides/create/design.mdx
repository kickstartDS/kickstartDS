...
This uses your `branding-token.json` as an input, and generates a folder `tokens` for you. That folder holds `JSON` files defining your Design Token by category:
...
To test that our integration works as expected we'll add the `css` platform for now, which will result in our Design Token being converted to CSS Custom Properties, as `src/token/token.css`.
...
If everything worked as expected, you should end up with a long `src/token/token.css` file and the following output on your terminal:
...
TODO This will leave us with the following files for now, at least if you've started afresh:
